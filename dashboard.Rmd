...
inputPanel(
  ...
  checkboxInput("rb", "Display journals", TRUE)
)
...
# plot
renderPlotly({
 p <- ggplot(jn_bx(), aes(y = prop, x = as.factor(year), colour = prop, label = journal_title)) +
  geom_boxplot() + 
   coord_flip() +
   theme_minimal() +
   xlab("Year") +
 ...
 plotly::ggplotly(p, tooltip = c("label", "y"), height = "300")
})
```
