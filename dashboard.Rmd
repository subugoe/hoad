...
renderPlotly({
  p <- ggplot(hybrid_sub(), aes(year, prop, fill = license)) +
    ...
    scale_fill_manual("License", values =  c("#c23531","#2f4554", "#61a0a8", "#d48265", "#91c7ae","#749f83")) +
...
tt <- reactive({
  jn_f() %>%
    filter(!is.na(country)) %>% # deal with missing country info provided by Open APC 
    count(country) %>% 
  mutate(prop = n / sum(n)) %>%
  arrange(prop) %>%
...
renderPlotly({
gg <- ggplot(tt(), aes(prop, country)) +
  geom_segment(aes(x = 0, y = country, xend = prop, yend = country), color = "grey50") +
  geom_point(color = "steelblue") +
  scale_x_continuous(expand=c(0,0.2), labels=percent, 
                     breaks=seq(0, 1, by=0.2)) +
  labs(x = "Articles funded per country (in percent)", y = NULL) +
  theme_minimal() 
ggplotly(gg)
})
