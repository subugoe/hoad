...
# access to data 
hybrid_df <- readr::read_csv("data/cr_hybrid_df_indicators.csv") %>%
...
tt <- o_apc_df %>%
    filter(journal %in% jn_f()$journal, year %in% jn_f()$year) %>%
     distinct(journal, year, oapc_n_year, .keep_all = TRUE) %>% 
  group_by(country) %>%
...
gg <- gg + labs(x=NULL, y=NULL, 
              #  title="Institutional spending on hybrid OA journal articles",
