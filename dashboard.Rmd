...
hybrid_df <- readr::read_csv("data/hybrid_publications.csv") %>%
  ...
  mutate(hybrid_type = factor(hybrid_type, levels = c("Open APC (Offsetting)", "Open APC (Hybrid)", "SCOAP<sup>3</sup>"))) %>%
...
### Text-mined author emails (first match per article) - Double click on domain!
...
### Hybrid OA licenses found per year (absolute)

```{r}
renderPlotly({
    ggplot(my_data(), aes(year, fill = license)) + 
    geom_bar(position = position_stack(reverse = TRUE)) +
    ggtitle(paste0("Selection: ", input$plot_clicked_data$name)) +
    scale_fill_manual(
      "License",
      values =  c(
        "#65BADA",
...
renderPlotly({
  p <- 
    ggplot(my_data(), aes(year, ..count.., fill = hybrid_type)) +
    geom_bar(position = position_stack(reverse = TRUE)) +
    ggtitle(paste0("Selection: ", input$plot_clicked_data$name)) +
    xlab("Year") +
    ylab("Articles") +
