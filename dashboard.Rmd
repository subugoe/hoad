...
renderPlotly({
  ...
  o_apc_sub <- o_apc_df %>%
    ...
    summarize(n_oa = sum(oapc_n_year, na.rm = TRUE)) %>%
    mutate(source = "Open APC")
  
  p <-
    ggplot(hybrid_sub, aes(year, n, fill = source, text = paste("Crossref:", n))) +
  ...
  p <-
    p + geom_bar(
      data = o_apc_sub,
      aes(year, n_oa, fill = hybrid_type, text = NULL),
      stat = "identity",
      alpha = 0.8
    )
  
  tt <- plotly::ggplotly(p, tooltip = "text") %>%
    layout(showlegend = TRUE, legend = list(font = list(size = 10)))
  tt$x$data[[2]]$text <-
    paste(tt$x$data[[2]]$name, ":", tt$x$data[[2]]$y, sep = ": ")
  tt$x$data[[3]]$text <-
    paste(tt$x$data[[3]]$name, tt$x$data[[3]]$y, sep = ": ")
  tt
})
```
