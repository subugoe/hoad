...
renderPlotly({
  ...
  o_apc_sub <- o_apc_df %>%
    ...
    group_by(year) %>% 
    summarize(oapc_n = sum(oapc_n_year, na.rm = TRUE))
  p <- ggplot(hybrid_sub, aes(year, licence_n)) +
    ...
    scale_y_continuous(labels=function(x) format(x, big.mark = " ", scientific = FALSE),
                       breaks= pretty_breaks()) +
    theme_minimal()
  p <- p + geom_bar(data = o_apc_sub, aes(year, oapc_n), stat = "identity", alpha = 0.8)  
  plotly::ggplotly(p)
})

o_apc_df %>% distinct(journal, year, oapc_n_year) %>% group_by(year) %>% summarize(sum(oapc_n_year, na.rm = TRUE))
```
