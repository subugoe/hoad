...
In the lower part of the dashboard page "Overview", the left-hand graph shows the extent to which information about funding of open access publication was available.  On the right, an overview about the first respective corresponding author email's top and lower-level domain is presented according to the publisher and journal selection.
...
The lower right chart presents email domains extracted from the open access full-texts per selected publisher or journal. Because the first email occurrence per article was obtained, the so parsed addresses roughly indicate the affiliation of the first or corresponding authors, a data point used to delineate open access funding. In the dashboard, a hierarchical, interactive treemap visualizes the email domains. Each top-level domain can be subdivided further to domain names representing academic institutions or companies. The size of each rectangle is proportional to the number of hybrid open access articles.
