...
`r dplyr::n_distinct(hybrid_cr$journal_title)` different subscription-based journals from `r dplyr::n_distinct(hybrid_cr$publisher)` different publishers between 2013 - 2017, accounting for about `r unlist(sum(hybrid_cr$license_ref_n, na.rm = TRUE) / (hybrid_cr %>% distinct(year, year_all) %>% summarize(sum(year_all))) * 100)`% of the overall journal article volume investigated.
...
Numbers and proprtion of hybrid open access journal articles varies across publishers and journals. In the year 2016, for example, the mean open access proportion per Springer Nature journal was `r my_df %>% filter(publisher == "Springer Nature") %>% .$prop %>% mean() * 100` % (SD = `r my_df %>% filter(publisher == "Springer Nature") %>% .$prop %>% sd() * 100` %), whereas Elsevier BV  mean open access articles proprtion per journal was `r my_df %>% filter(publisher == "Elsevier BV") %>% .$prop %>% mean() * 100` % (SD = `r my_df %>% filter(publisher == "Elsevier BV") %>% .$prop %>% sd() * 100` %). Figure 3 shows the variations for the five publishers with the largest number of hybrid open access journals in 2016 according to our data.
