...
[This open source dashboard](https://subugoe.shinyapps.io/hybridoa/) presents the uptake of hybrid open access for `r dplyr::n_distinct(hybrid_cr$journal_title)` subscription-based journals from `r dplyr::n_distinct(hybrid_cr$publisher)` publishers between 2013-2019. During this seven years period, these journals published `r hybrid_cr %>% distinct(issued, yearly_all) %>% summarize(n = sum(yearly_all)) %>% .$n` articles. `r dplyr::n_distinct(hybrid_cr$doi_oa)` articles were made openly available without delay, representing a hybrid open access share of `r unlist(dplyr::n_distinct(hybrid_cr$doi_oa) / (hybrid_cr %>% distinct(issued, yearly_all) %>% summarize(prop = sum(yearly_all))) * 100)`%.
