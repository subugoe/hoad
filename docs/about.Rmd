...
`r dplyr::n_distinct(hybrid_cr$journal_title)` subscription-based journals from `r dplyr::n_distinct(hybrid_cr$publisher)` publishers between 2013 - 2019. These hybrid open access articles represent `r unlist(dplyr::n_distinct(hybrid_cr$doi_oa) / (hybrid_cr %>% distinct(issued, yearly_all) %>% summarize(prop = sum(yearly_all))) * 100)` percent of published articles (`r hybrid_cr %>% distinct(issued, yearly_all) %>% summarize(n = sum(yearly_all)) %>% .$n` articles).
