...
Recently, [Unpaywall](https://unpaywall.org/) from [Impactstory](https://github.com/impactstory) has become a widely used discovery service for open access content including hybrid open access. This openly available service, which is integrated in major bibliographic databases like the Web of Science or Scopus, determines open access status information and links to freely available full-texts. In the case of  open access provided by publishers, Unpaywall searches Crossref, a DOI registration agency for scholarly works, and publisher websites for openly available full-texts and corresponding license statements. Following @Piwowar_2017, bibliometric analysis re-using Unpaywall data often define hybrid open access as publisher provided open access with open content license statements, which were not published in fully open access journals contained in the [Directory of Open Access Journals (DOAJ)](https://doaj.org/) (e.g. @Bosman_2018 or the [German Open Access Monitor](https://open-access-monitor.de/#/)). However, using Unpaywall to discover hybrid open access is limited in two respects: first, there are some fully open access journals, which are not indexed in the DOAJ [@Rimmert_2017], presumably because these journals did not comply with the comprehensive DOAJ inclusion criteria. Secondly, Unpaywall focusses on current open access provision, but does not track when the open access articles was made openly available, making it not possible to distinguish between immediate and delayed open access provision. Although Unpaywall has greatly improved the discoverability of open access in general, its applicability to hybrid open access defined as immediate open access of individual articles in subscription-based journals remains limited for the above two reasons.

Along with the discoverability of hybrid open access, there is also considerable debate about the revenue sources of this publishing model. In addition to subscriptions, hybrid open access journals often charge publication fees, also known as article processing charges (APC), to provide immediate open access to individual articles [@Suber_2012]. Although it was initially envisioned that with growing funding opportunities for publication fees publishers would transition hybrid open access journals to fully open access [@Prosser_2003], it remains unclear to which extent the increasing willingness to pay for open access contributed to it, and how cost-effective these spendings were [@Bj_rk_2014; @Pinfield_2017]. 

The [Open Access 2020 Initiative](https://oa2020.org/) has taken up the problem of missing evidence, calling for a transparent approach to converting budgets spent for subscriptions to open access business models. Likewise, funders and libraries have developed compliance criteria to improve reporting practices around hybrid open access publishing in the last years. The Sponsoring Consortium for Open Access Publishing in Particle Physic (SCOAP^3), for example, archives funded articles in several formats including metadata in a dedicated repository. In the US, Chorus, a non-profit serving more than 50 publishers, uses freely available metadata from Crossref to present open access compliance for dedicated funders with [interactive dashboards](https://dashboard.chorusaccess.org/). In Europe, the Wellcome Trust not only [automatically checks](https://compliance.cottagelabs.com/docs) if authors and publishers comply with a comprehensive set of metadata obligations, but also contribute to transparency of funder actions by making expenditures for open access publications openly available [@Kiley_2015]. Other European research funders and research-performing institutions including the Austrian Science Fund FWF and British universities disclose spendings for open access articles in hybrid open access journals as well [@fwf_apc_13; @jisc_14]. Because Crossref indexes most articles where institutions sponsored publication fees, its metadata services can be used to make self-reported spendings about open access support comparable at the journal and publisher level [@Jahn_2016]. The [Open APC Initiative](https://github.com/openapc/openapc-de), which collects and standardizes openly available spending data together with crowd-sourced expenditures, is an example for using Crossref metadata to increase the transparency of publication fee spending across funders and institutions [@Pieper_2018].  <!-- When merging various cost datasets, curation issues can be addressed using Crossref metadata [@Jahn_2016].-->

In the light of a perceived slow and ineffective growth of open access,  the cOAlition S, a network of 11 research funders, released its widely discussed [Plan S](https://www.coalition-s.org/) to "accelerate the transition to full and immediate Open Access" in September 2018. Starting from 2020, members of the cOAlition S only wants to fund hybrid open access articles published under a transformative agreement, in which spendings for subscription and open access publication are considered together. The Plan S also requires a commitment to full open access transition from the publishers. The cOAlition S refers to the [ESAC Initiative](http://esac-initiative.org/), based at the German Max Planck Digital Library (MPDL) focusing on standardizing open access workflows between publishers and research institutions, to observe these contracts. In 2017, ESAC released guidance about how to implement transformative agreements. According to these guidelines, publishers have to ensure that only corresponding authors affiliated with the agreement institution are eligible for open access support. Furthermore, publishers need to provide rich metadata to Crossref including open access license information and reports about covered articles to the agreement institutions [@Geschuhn_2017]. Some library consortia like the Swedish Bibsam or the British Jisc have started to make these reports openly available with the Open APC Initiative [@Lund_n_2018; @Pieper_2018].  

These developments show that open and standardized reporting is critical to improve the discoverability and transparency of hybrid open access publishing. To encourage good data practices around the transition of subscription-based journals to open access, we propose the following characteristics hybrid open access journals must have to become included in monitoring exercises:
...
Information about corresponding authors play a crucial role in funding open access publication. Because Crossref metadata did not cover affiliations for all articles in our sample, and if available, lack standardisation, we text-mined hybrid open access articles in our sample for email addresses. Interfacing the [Crossref TDM service](https://support.crossref.org/hc/en-us/articles/215750183-Crossref-Text-and-Data-Mining-Services) using the [crminer](https://github.com/ropensci/crminer) R-client from rOpenSci [@crminer], we downloaded the full-texts and extracted the first email occurrence per article. With the help of [urltools](https://CRAN.R-project.org/package=urltools) [@urltools], we extracted parts of the email domain, which can be used as rough approximation of the affiliation of the first resp. corresponding author at the time of publication. To avoid mis-use, particularly academic spamming, source code and data used for this text-mining exercise are currently stored in a privately git-repository until it is secured that no full author emails are accidentally shared.

### Data Accuracy

Notice that our estimates about the extent of hybrid open access publishing are likely to be conservative. Not all authors and institutions share information about hybrid open access spending with the Open APC initiative. Similarly, not all publishers shared comprehensive metadata about access and re-use with Crossref. To assess data accuracy, we first describe how many hybrid open journals covered by the Open APC initiative provided license statements via Crossref between 2013 - 2019. Using Unpaywall data, we were also able to determine additional articles with open content license statements and compared them with our sample. Next, we evaluated the accurary of our automated methods using article random samples. Recall and precision for obtaining hybrid open access articles from Crossref and for extracting email addresses from full-texts were investigated.
