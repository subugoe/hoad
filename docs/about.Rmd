...
The journal selection criteria used in the dashboard has some limitations that likely yield to the comparatively lower estimates. Only one out of three publishers, where a research institution or a funder has demonstrably sponsored a hybrid open access article, shared licensing metadata through Crossref as suggested by initiatives like [ESAC](http://esac-initiative.org/). Particularly, smaller publishing houses were not well represented in the sample. Similarly, @Piwowar_2017 found that Crossref lacks some license coverage. Therefore, they investigated publisher websites for license statements as well. Furthermore, @Piwowar_2017 observed that the percentage of open access provided by publishers, which was free to read, but where the journal was neither listed in the DOAJ nor provided licensing metadata, was the most common method to provide open access among publishers. 

In addition to the incomplete licensing coverage, our findings also suggest a gap between spending information available through the Open APC initiative and the total number of hybrid open access articles that have been registered with Crossref in recent years. One reason is that reporting to the Open APC initiative is voluntary [@Jahn_2016]; another that despite its international focus, most spending information was collected from European institutions, in particular from the UK [@Pieper_2018]. However, not all hybrid open access articles were sponsored by institutions, but authors can make use of other resources to publish open access, or fees are waived [@Solomon_2011]. In case of transformative agreements, Open APC has a strong focus on  consortia who established agreements with Springer Nature. As the example of the Sponsoring Consortium for Open Access Publishing in Particle Physic -- SCOAP$^3$ shows, consortia also use repositories to disclose hybrid open access articles they sponsored.  Therefore, there is a chance that some hybrid open access journals and articles were not detected using data from the Open APC initiative alone.


An obvious direction for future work is to broaden the 
- discpline-specific variations (f1000 paper)
- track down to speicifc contracts -> esac registry
- use pre-defined journal packages, esac 
- combine with bibliometric databases (stahlschmidt)
- compare it to the overall market share

lastly address subject-speific 

Plan S is overall tool track ESAC-compliant journals trna
